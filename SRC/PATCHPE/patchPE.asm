...
local	wSectionSize:WORD
	...
	.if (CARRY? || (ax != cx))
		invoke StringOutX, addr text5
		jmp exit
	.endif
	.if (word ptr MZ_Hdr+0 != "ZM")
		invoke StringOutX, addr text6
		jmp exit
	.endif
if 0
	...
	.if ((PE_Hdr.Signature == "EP") || (PE_Hdr.Signature == "XP"))
		...
		.if (ax > IMAGE_OPTIONAL_HEADER.Subsystem)
			.if (PE_Hdr.OptionalHeader.Subsystem == IMAGE_SUBSYSTEM_WINDOWS_GUI)
				invoke StringOutX, CStr(<"aborted, because this is a Windows GUI app!",13,10>)
				jmp exit
			.endif
		.endif
		.if (bCodeWriteable)
