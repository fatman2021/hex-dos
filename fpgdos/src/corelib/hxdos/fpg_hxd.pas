...
type
  ...
  protected
    ...
    function    DoWindowToScreen(ASource: TfpgWindowBase; const AScreenPos: qtwinapi.TPoint): TPoint; override;
...
//    hcr_dir_swne: HCURSOR;
  ...
  public
    ...
    function    GetScreenPixelColor(APos: qtwinapi.TPoint): TfpgColor; override;
  ...
  { Used mainly for receiving drags - being the target of the drag }
   ...
   private
     ...
     FRegistered: boolean;
     { IDropTarget }
     function    DragEnter(const dataObj: IDataObject; grfKeyState: DWORD; pt: qtwinapi.TPoint; var dwEffect: DWORD): HResult;StdCall;
     function    DragOver(grfKeyState: DWORD; pt: qtwinapi.TPoint; var dwEffect: DWORD): HResult;StdCall;
     function    DragLeave: HResult;StdCall;
     function    Drop(const dataObj: IDataObject; grfKeyState: DWORD; pt: qtwinapi.TPoint; var dwEffect: DWORD):HResult;StdCall;
   public
...
function GET_X_LPARAM(lp : qtwinapi.LParam) : longint;
begin
  result:=smallint(LOWORD(lp));
end;

function GET_Y_LPARAM(lp : qtwinapi.LParam) : longint;
begin
...
begin
  ...
  {$ELSE}
  wg := TfpgWidget(hxuser.GetWindowLong(wh, GWL_USERDATA));
...
begin
  ...
  rec.Bottom := aRect.Bottom;
  SP := RadialPoint(Angle1, rec);
  if Angle2 + Angle1 > 360*16 then
    Angle2 := (Angle2 + Angle1) - 360*16
  else
    Angle2 := Angle2 + Angle1;
  EP := RadialPoint(Angle2, rec);
  SX := SP.X;
  SY := SP.Y;
  EX := EP.X;
  EY := EP.Y;
...
begin
  ...
  begin
    ...
    {$ELSE}
    hxuser.SetWindowLong(hwnd, GWL_USERDATA, longword(w));
    {$ENDIF}
  end
...
//      writeln('ALL');
  ...
  {$ELSE}
  w      := TfpgGDIWindow(hxuser.GetWindowLong(hwnd, GWL_USERDATA));
  {$ENDIF}
  Result := 0;
  ...
  begin
    ...
    Result := hxuser.DefWindowProc(hwnd, uMsg, wParam, lParam);
    Exit; //==>
  end;
        ...
        begin
          ...
          *)
          ...
          begin
            ...
            {$ELSE}
            mw := TfpgGDIWindow(hxuser.GetWindowLong(h, GWL_USERDATA));
            {$ENDIF}
          end;
...
//            if not (PopupListFirst is TfpgPopupWindow) then
    ...
    WM_PAINT:
        begin
          ...
          {$ENDIF}
          *)
          hxuser.BeginPaint(w.WinHandle, {@}PaintStruct);
          fpgSendMessage(nil, w, FPGM_PAINT, msgp);
          hxuser.EndPaint(w.WinHandle, {@}PaintStruct);
        end;
    ...
    WM_SYSCOMMAND:
        begin
          if wParam = ID_ABOUT then
            ...
            hxuser.DefWindowProc(hwnd, uMsg, wParam, lParam);
        end
    else
      Result := hxuser.DefWindowProc(hwnd, uMsg, wParam, lParam);
  end;
...
begin
  ...
  hxuser.PostMessage(TfpgGDIWindow(MainForm).WinHandle, WM_NULL, 0, 0);
end;
...
begin
  ...
  begin
    ...
    hxuser.RegisterClass(HiddenWndClass);
...
begin
  ...
  begin
    ...
    hIcon         := LoadIconA(hInstance, 'MAINICON');
    hCursor       := LoadCursor(0, IDC_ARROW);
    hbrBackground := 0; //COLOR_WINDOW;
    lpszClassName := 'FPGWIN';
  end;
  hxuser.RegisterClass(WindowClass);
  ...
  hxuser.RegisterClass(WidgetClass);
...
//  hcr_dir_swne  := LoadCursor(0, IDC_SIZENESW);
  ...
  ActivationHook := SetWindowsHookExA(WH_CBT, HOOKPROC(@fpgCBTProc), 0, GetCurrentThreadId);
...
var
  Msg: TMsg;
begin
  Result := hxuser.PeekMessageW(Msg, 0, 0, 0, PM_NOREMOVE);
end;
...
begin
  ...
  if (GetVersion() < $80000000) then
    hxuser.GetMessage(Msg, 0, 0, 0)   //NT
  else
    hxuser.GetMessage(Msg, 0, 0, 0);   //Win98
  {$ENDIF}

  hxuser.DispatchMessage(@msg);
end;
...
begin
  ...
  GetVesaModeInfo(GetVesaMode,Info);
  Result := Info.XResolution;
...
begin
  //GetWindowRect(GetDesktopWindow, r);
  GetVesaModeInfo(GetVesaMode,Info);
  Result := Info.YResolution;
  // Result := hxuser.GetSystemMetrics(SM_CYSCREEN);
end;
...
var
  c: longword;
begin
  c := hxgdi.GetPixel(FDisplay, APos.X, APos.Y);
...
begin
  ...
  if uMsg = WM_MOUSEWHEEL then
    hxuser.ScreenToClient(FWinHandle, @pt)
  else
    hxuser.ClientToScreen(FWinHandle, @spt);
...
begin
  ...
  FWinHandle := hxuser.CreateWindowExW(
...
begin
  if FWinHandle <= 0 then
           Exit;
  hxuser.DestroyWindow(FWinHandle);
  FWinHandle := 0;
end;
...
begin
  ...
  begin
    ...
    if FWindowType in [wtPopup] then
      hxuser.ShowWindow(FWinHandle, SW_SHOWNOACTIVATE)
    else
      hxuser.ShowWindow(FWinHandle, SW_SHOWNORMAL);
    ...
    hxuser.UpdateWindow(FWinHandle);
    FSkipResizeMessage := False;
  end
  else
    hxuser.ShowWindow(FWinHandle, SW_HIDE);
end;
...
begin
  ...
  if UnicodeEnabledOS then
    hxuser.SetWindowTextW(WinHandle, PWideChar(Utf8Decode(ATitle)))
  else
    hxuser.SetWindowTextA(WinHandle, PChar(Utf8ToAnsi(ATitle)));
  {$endif}
end;
...
begin
  hxuser.SetCapture(FWinHandle);
end;

procedure TfpgGDIWindow.ReleaseMouse;
begin
  hxuser.ReleaseCapture;
//  if PopupListFirst <> nil then
//    hxuser.SetCapture(PopupListFirst^.);
...
begin
  ...
  begin
    ...
    hxuser.InvalidateRect(WinHandle, nil, True);
    FSkipResizeMessage := False;
  end;
end;
...
begin
  ...
  destdc := hxuser.GetDC(TfpgWindow(FWindow).WinHandle);
  srcdc := hxuser.GetDC(TfpgWindow(TfpgGDICanvas(ASrcCanvas).FWindow).WinHandle);
...
begin
  ...
  begin
    FDrawWindow := TfpgGDIWindow(awin);
    FWinGC      := hxuser.GetDC(FDrawWindow.FWinHandle);
    ...
    begin
      ...
      begin
        TryFreeBackBuffer;
//        DoGetWinRect(ARect);
        FBufferBitmap := hxgdi.CreateCompatibleBitmap(FWinGC, ARect.Width, ARect.Height);
        FBufgc        := hxgdi.CreateCompatibleDC(FWinGC);
        Fgc           := FBufgc;
      end;
...
begin
  ...
  begin
    ...
    hxuser.ReleaseDC(FDrawWindow.FWinHandle, FWingc);
...
begin
  hxgdi.SetPixel(Fgc, X, Y, fpgColorToWin(AValue));
end;
...
begin
  ...
  {$IFNDEF wince}
  if a2 < 0 then
    hxgdi.SetArcDirection(FGc, AD_CLOCKWISE)
  else
    hxgdi.SetArcDirection(FGc, AD_COUNTERCLOCKWISE);
  {$ENDIF}
  Angles2Coords(x, y, w, h, a1*16, a2*16, SX, SY, EX, EY);
  {$IFNDEF wince}
  hxgdi.Arc(Fgc, x, y, x+w, y+h, SX, SY, EX, EY);
  {$ENDIF}
end;
...
begin
  ...
  Angles2Coords(x, y, w, h, a1*16, a2*16, SX, SY, EX, EY);
  {$IFNDEF wince}
  hxgdi.Pie(Fgc, x, y, x+w, y+h, SX, SY, EX, EY);
  {$ENDIF}
end;
...
//  pts: array of TPoint;
begin
  hxgdi.Polygon(Fgc, Points, NumPts);
end;
...
begin
  hxgdi.MoveToEx(Fgc, x1, y1, nil);
  hxgdi.LineTo(Fgc, x2, y2);
end;
...
begin
  ...
  begin
    ...
    {$ELSE}
    hxuser.FrameRect(Fgc, wr, FBrush); // this handles 1x1 rectangles
    {$ENDIF}
  end
  else
  begin
...
begin
  ...
  {$ifdef wince}
  hxgdi.ExtTextOut(Fgc, x, y, ETO_CLIPPED, nil, PWideChar(WideText), Length(WideText), nil);
  {$else}
  hxgdi.TextOutW(Fgc, x, y, PWideChar(WideText), Length(WideText));
  {$endif}
end;
...
type
  PCDot = ^TCDot;
  TCDot = array[1..2] of DWORD;
  PCDash = ^TCDash;
  TCDash = array[1..4] of DWORD;
const
  cDot: TCDot = (1, 1);
  cDash: TCDash = (4, 2, 4, 2);
var
  lw: integer;
  logBrush: TLogBrush;
  cDotPtr: PCDot;
  ...
  case AStyle of
    lsDot:
      begin
        CDotPtr^ := CDot;
        {$IFNDEF wince}
        FPen := ExtCreatePen(PS_GEOMETRIC or PS_ENDCAP_FLAT or PS_USERSTYLE, FLineWidth, logBrush, Length(cDot), DWORD(cDotPtr));
        {$ENDIF}
      end;
    lsDash:
      begin
        {$IFNDEF wince}
        CDashPtr^ := CDash;
        FPen := ExtCreatePen(PS_GEOMETRIC or PS_ENDCAP_FLAT or PS_USERSTYLE, FLineWidth, logBrush, Length(cDash), DWORD(cDashPtr));
        {$ENDIF}
      end;
    lsSolid:
      begin
...
begin
  hxgdi.SetTextColor(Fgc, fpgColorToWin(cl));
end;
...
begin
  ...
  hxgdi.Rectangle(Fgc, x, y, x + w + 1, y + h + 1);
...
begin
  ...
  {$else}
  hxgdi.GetTextExtentPoint32W(wapplication.display, PWideChar(WideText), Length(WideText), ts);
  {$endif}
...
begin
  Result := '';
  if not hxuser.OpenClipboard(0) then
    Exit;

  h := GetClipboardData(CF_TEXT);
  if h <> 0 then
  begin
    p := hxkernel.GlobalLock(h);
    FClipboardText := '';
    while p^ <> #0 do
    begin
...
begin
  ...
  begin
    ...
    begin
      drvs := chr(n+ord('A'))+':\';
      {$IFNDEF wince}
      if hxkernel.GetDriveTypeA(PChar(drvs)) <> 1 then
      begin
    ...
    *)
    ...
    begin
      ...
      *)
      ...
      begin
        New(F);
        F^.cfFormat := RegisterClipboardFormatA(PChar(itm.format));
        F^.ptd := nil;
        F^.dwAspect := DVASPECT_CONTENT;
        F^.lindex := -1;
        F^.tymed := TYMED_HGLOBAL;
...
function TGDIDragManager.DragEnter(const dataObj: IDataObject;
  grfKeyState: DWORD; pt: qtwinapi.TPoint; var dwEffect: DWORD): HResult; StdCall;
begin

end;

function TGDIDragManager.DragOver(grfKeyState: DWORD; pt: qtwinapi.TPoint;
...
function TGDIDragManager.Drop(const dataObj: IDataObject; grfKeyState: DWORD;
  pt: qtwinapi.TPoint; var dwEffect: DWORD): HResult; StdCall;
begin
...
begin
  ...
  if FEnabled then
  begin
    FHandle := hxuser.SetTimer(0, 0, Interval, @TimerCallBackProc);
  end
  else
  begin
    if FHandle <> 0 then
    begin
      hxuser.KillTimer(FHandle, 0);
      FHandle := 0;
    end;
  end;
end;
