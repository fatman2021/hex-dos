...
uses
  hxkernel,hxuser,hxgdi,hxole32, Classes, {ActiveX,} hxshell32, fpg_base;
...
type
  ...
  private
    ...
    procedure   DoDragDrop(DataObject: IDataObject; DropSource: IDropSource; dropeffect: Integer; eff: PtrInt);
  public
    constructor Create;
    destructor  Destroy; override;
    procedure   Execute;
...
var stgm: PStgMedium absolute medium;
begin
  Dispose(stgm);
end;
...
begin
  ...
  begin
    ...
    IsTranslated := False;
    Result := RegisterClipboardFormatA('text/html');
  end
  ...
  begin
    IsTranslated := False;
    Result := RegisterClipboardFormatA(PChar(AMime));
  end;
end;
...
begin
  ...
  begin
    lName := '';
    i := GetClipboardFormatNameA(FE.cfFormat,FormatName,MAX_PATH);
    if i <> 0 then
    begin
      lName := FormatName;
    end
...
procedure TDragFilesSource.DoDragDrop(DataObject: IDataObject; DropSource: IDropSource; dropeffect: Integer; eff: PtrInt);
begin
...
begin
  ...
  dwResult := DoDragDrop(DataObject as IDataObject, DropSource as IDropSource, DROPEFFECT_COPY, @dwEffect);
...
begin
  //RegisterDragDrop(TfpgWidget(FDropTarget).WinHandle, Self as IDropTarget);
  FDropTarget := TfpgWidget(FDropTarget).WinHandle;
  FRegistered := True;
end;
