...
var
  {$ifdef HX_DOS}
  FPG_DEFAULT_FONT_DESC: string = 'Sanford Book-8:antialias=true';
  FPG_DEFAULT_SANS: string = 'Sanford Book';
  {$endif}
  {$IFDEF WINDOWS}
...
type
  ...
  protected
    ...
    function    DoWindowToScreen(ASource: TfpgWindowBase; const AScreenPos: {$ifdef HX_DOS}qtwinapi.{$endif}TPoint): {$ifdef HX_DOS}qtwinapi.{$endif}TPoint; virtual; abstract;
...
{ Points }
procedure SortRect(var ARect: TRect); overload;
procedure SortRect(var ARect: TfpgRect); overload;
procedure SortRect(var left, top, right, bottom: integer); overload;
{$ifdef HX_DOS}
function ToTPoint(x,y: Integer): qtwinapi.TPOINT;
{$else}
function ToTPoint(x,y: Integer): TPoint;
{$endif}
