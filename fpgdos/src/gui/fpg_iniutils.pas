...
type
  ...
  public
    ...
    function    ReadLongword(const ASection, AIdent: string; ADefault: longword): longword;
    ...
    procedure   WriteLongword(const ASection, AIdent: string; Value: Longword);
  end;
...
type
  TConvInt64ToLongWord = record
    case integer of
     0: (Toint64: int64);
     1: (Tolongword: longword);
  end;

function TfpgINIFile.ReadLongword(const ASection, AIdent: string; ADefault: longword): longword;
var AValue: int64; AConv: TConvInt64ToLongWord;
begin
  AValue := ADefault;
  if (not ValueExists(ASection, AIdent)) and
    (not FReadOnly) then
    WriteInteger(ASection, AIdent, ADefault);
  AValue := inherited ReadInt64(ASection, AIdent, AValue);
  AConv.Toint64 := AValue;
  Result := AConv.Tolongword;
end;
...
procedure TfpgIniFile.WriteLongword(const ASection, AIdent: string; Value: Longword);
begin
  inherited WriteInt64(ASection, AIdent, Value);
end;
